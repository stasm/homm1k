...
// the neighbor with a lower distance score is selected. Note that this function
// also updates the `next` global every time it's called, which is used in the
// game loop to move the player. Path tracing starts at the target and proceeds
// towards the player which means that the last time `next` is updated it will
// hold the index of the tile which is the closest to the player and on the path
// to the target.
trace = i => {
    while (i != player) {
        viewport(next = i, 00000001000300000); // The dot
        i = neighbors(i).reduce((acc, cur) =>
            world[cur] < world[acc] ? cur : acc, i);
    }
}
